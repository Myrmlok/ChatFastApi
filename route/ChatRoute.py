...
async def getChat(chat_id:int,user:UserApp=Depends(get_current_user)):
    return ChatMapper.entity_to_dto_withDepends( await ChatService.get_chat(chat_id,user))
...
async def update_chat(chat_id:int,dto:ChatDto,user:UserApp=Depends(get_current_user)):
    ...
    entity.id=chat_id
    return ChatMapper.entity_to_dto(await ChatService.update_chat(entity,user))
@chat_route.post("/{chat_id}/users/{user_id}")
async def add_user_to_chat(chat_id:int,user_id:str,user:UserApp=Depends(get_current_user)):
    return ChatMapper.entity_to_dto_withDepends(await ChatService.add_user_to_chat(chat_id,user,uuid.UUID(user_id)))
